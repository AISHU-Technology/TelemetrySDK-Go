...
func NewEvent(typea string) common.AREvent {
	return common.NewEvent(typea)
}

// NewAttribute 创建新的 Attribute 。
func NewAttribute(key string, value common.Value) *common.Attribute {
	return common.NewAttribute(key, value)
}

// BoolValue 传入 bool 类型的值。
func BoolValue(value bool) common.Value {
	return common.BoolValue(value)
}

// BoolArray 传入 []bool 类型的值。
func BoolArray(value []bool) common.Value {
	return common.BoolArray(value)
}

// IntValue 传入 int 类型的值。
func IntValue(value int) common.Value {
	return common.IntValue(value)
}

// IntArray 传入 []int 类型的值。
func IntArray(value []int) common.Value {
	return common.IntArray(value)
}

// FloatValue 传入 float64 类型的值。
func FloatValue(value float64) common.Value {
	return common.FloatValue(value)
}

// FloatArray 传入 []float64 类型的值。
func FloatArray(value []float64) common.Value {
	return common.FloatArray(value)
}

// StringValue 传入 string 类型的值。
func StringValue(value string) common.Value {
	return common.StringValue(value)
}

// StringArray 传入 []string 类型的值。
func StringArray(value []string) common.Value {
	return common.StringArray(value)
}

// NewLink 创建新的 Link 。
func NewLink(ctx trace.SpanContext) common.Link {
	return common.NewLink(ctx)
}
